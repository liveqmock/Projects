...
public class UserLoginFilter implements javax.servlet.Filter {
	 ...
	 * 登录主页面文件
	 */
	private String login_main_page;
	...
	public void init(FilterConfig filterconfig) throws ServletException {
		...
		//发布流程
//		PublishingProcess process = PublishingProcess.getInstance();
//		process.publishing();
		...
		login_main_page = filterconfig.getInitParameter("login_main_page");
		...
		log.debug("[logon_page,loginPageName,loginActionName,main_page]:" + logon_page+","+login_main_page+","+loginActionName+","+main_page);
		if (null == logon_page) {
	...
	private boolean isInCommonRight(String url){
		...
		try{
			...
			if(main_page.equals(url)||url.startsWith(login_main_page)||logon_page.equals(url)||error_page.equals(url)||loginActionName.equals(url)){
				//||url.startsWith(common_page_prefix)
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if("".equals(para.exportExcelPath)){
			java.util.Properties prop = System.getProperties();		
			String os = prop.getProperty("os.name");		
			para.exportExcelPath = req.getSession().getServletContext().getRealPath("download/quotations/")+ java.io.File.separator;
			para.exportWordPath = req.getSession().getServletContext().getRealPath("download/ContractTemplates/")+ java.io.File.separator;
			para.templatesExcelPath = req.getSession().getServletContext().getRealPath("upload/quotations/")+ java.io.File.separator;
			para.templatesWordPath = req.getSession().getServletContext().getRealPath("upload/ContractTemplates/")+ java.io.File.separator;
			para.purchaseWordPath = req.getSession().getServletContext().getRealPath("download/purchaseOrderFile/")+ java.io.File.separator;
			para.provedPath = req.getSession().getServletContext().getRealPath("upload/ArrivalProved/")+ java.io.File.separator;
			para.contractTemporaryPath = req.getSession().getServletContext().getRealPath("upload/ContractTemporary/")+ java.io.File.separator;
			para.supplierQuotationsPath = req.getSession().getServletContext().getRealPath("upload/supplierQuotations/")+File.separator;
			if(os.startsWith("win") || os.startsWith("Win")) {//windows操作系统
				para.exportExcelPath = para.exportExcelPath.replaceAll("\\\\","//");
				para.exportWordPath = para.exportWordPath.replaceAll("\\\\","//");
				para.templatesExcelPath = para.templatesExcelPath.replaceAll("\\\\","//");
				para.templatesWordPath = para.templatesWordPath.replaceAll("\\\\","//");
				para.purchaseWordPath = para.purchaseWordPath.replaceAll("\\\\","//");
				para.provedPath = para.provedPath.replaceAll("\\\\","//");
				para.contractTemporaryPath = para.contractTemporaryPath.replaceAll("\\\\","//");
				para.supplierQuotationsPath = para.supplierQuotationsPath.replaceAll("\\\\", "//");
			...
			LoginWorkingThread loginWork = new LoginWorkingThread();
			Thread t = new Thread(loginWork);
			t.start();
		}
		try {
		...
		try{
			...
			if(tbUser == null){
				...
				if(url.equals(loginActionName)||request_uri.indexOf(logon_page)!=-1||request_uri.equals(ctxPath+"/")|| url.equals(verificationCodeUrl)){
				...
				//根据配置文件的值判断是否进行权限
				boolean isAuthorizedRight = appConfig.getString("AuthorizedRight").equals("true")?true:false;
				if(isAuthorizedRight){
