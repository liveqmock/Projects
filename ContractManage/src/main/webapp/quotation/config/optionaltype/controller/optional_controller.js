...
function saveAndUpdateQuotation() {
	...
	var rows = $('#ttt').datagrid('getRows');
	var json = "[";
	var catalogTotal = 0.0;
	var afterDiscount = 0.0;
	var afterDiscountNow = 0.0;
	var totalmoneys = 0.0;
	//循环给提交删除参数赋值(音乐风格编码)
	$.each(rows, function (i, n) {
		...
		catalogTotal = catalogTotal+n.totalprice;
		afterDiscount = afterDiscount+n.rebateprice;
		afterDiscountNow = afterDiscountNow+n.rebateafter;
		totalmoneys = totalmoneys+n.total;
		json += "{'id':'"+n.id+"','sourceDataId':'"+n.dataRecordId+"','configId':'"+configsIds+"','number':'"+n.quantity+"','discount':'"+discountRate+"','firstYear':'"+n.firstYear+"','secondYear':'"+n.secondYear+"','thirdYear':'"+n.thirdYear+"','catalogPrice':'"+n.totalprice+"','discountedPrice':'"+n.rebateprice+"','discountAfterPrice':'"+n.rebateafter+"','totalPrice':'"+n.total+"'}";
		if(i!=rows.length-1){
			json += ',';
		}
	});
	...
	json += "]";
	$.post(ctx+ 'quotations/quotations/quotations!saveAndUpdateQuotation.action', {
		id : json,
		catalogTotalprice:catalogTotal,	  //目录合价
		afterDiscountPrice:afterDiscount,	//折扣后价格
		afterDiscountNowPrice:afterDiscountNow,	//折扣后现场价
		totalPrice:totalmoneys			//合计
	}, function(data) {
