...
$(function(){
	$('#tt').datagrid({
		...
		columns:[[  		   
					...
					{field:'createname',title:'创建者',width:80,align:'center',
						formatter:function(value,rec,index){
							 return 'admin';
						 }
					},	
					...
					{field:'operate',title:'操作',width:80,align:'center',
							formatter:function(value,rec,index){
								var id=rec.id;
								var edit;
								edit='<a href="#" onclick="deleteConfigModels(\''+id+'\')">删除</a>&nbsp;|&nbsp;<a href="#" onclick="updateModels(\''+id+'\')">修改</a>&nbsp;|&nbsp;<a href="#" onclick="updateModelsInfos(\''+id+'\',\''+rec.dataSourceId+'\',\''+rec.dataSourceName+'\')">查看详情</a>';
								return edit;
							}
						}
					]],
